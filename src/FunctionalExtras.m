...
Assert; Let; Every; Some; Second; Reshape; FullTranspose; WithAll; Collapse; ThreadThrough; MapThrough;
...
CollapseHashMap[data_, by_List, fs_] :=
	Let[{
			gathered=GatherBy[data,Map[#,by]&],
			collapsed=MapThread[#1[#2]&,{fs,Transpose[#]}]& /@ gathered},
		collapsed];


ThreadThrough[x_, forms_List : {Identity}] :=
	(Composition@@Reverse[forms])[x];


MapThrough[x_, forms_List : {Identity}, levels_ : {1}] :=
	Map[Composition@@Reverse[forms], x, levels];
